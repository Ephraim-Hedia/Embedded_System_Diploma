...
MYRTOS_errorID MYRTOS_AcquireMutex(Mutex_ref* Mref , Task_ref* Tref)
{
	...
	else
	{
		...
		else //There is a pending request by ant task
		{
			return MutexReachedToMaxNymberOfUsers;
		}
	}
	return NoError;
}
...
//void MYRTOS_InitMutexBuff(FIFO_Buf_t * Mutex_QUEUE , Task_ref* Mutex_QUEUE_FIFO)
//{
//	FIFO_init(Mutex_QUEUE, &Mutex_QUEUE_FIFO, 10);
//}
//
//MYRTOS_errorID MYRTOS_AcquireMutex(FIFO_Buf_t * Mutex_QUEUE, Task_ref* Tref )
//{
//	FIFO_Buf_Status FifoError ;
//
//	FifoError =FIFO_enqueue(Mutex_QUEUE, Tref) ;
//
//	if(FifoError == FIFO_null)
//		return Mutex_Queue_NULL;
//	else if(FifoError == FIFO_full)
//		return MutexReachedToMaxNymberOfUsers;
//	else if (FifoError == FIFO_no_error)
//	{
//		if((Mutex_QUEUE->count) > 1)
//		{
//			Tref->TaskState=Suspend;
//			//to be suspend immediately
//			MYRTOS_OS_SVC_SET(SVC_TerminateTask);
...
//
//}
//MYRTOS_errorID MYRTOS_ReleaseMutex(FIFO_Buf_t * Mutex_QUEUE,Task_ref* Tref)
//{
//	FIFO_Buf_Status FifoError ;
//	FifoError =FIFO_dequeue(Mutex_QUEUE, &Tref) ;
//
//	if(FifoError == FIFO_null)
//		return Mutex_Queue_NULL;
//	else if (FifoError == FIFO_empty)
//		return Mutex_Queue_Empty;
//	else if (FifoError == FIFO_no_error)
//	{
//		Tref->TaskState=Waiting;
//		MYRTOS_OS_SVC_SET(SVC_ActivateTask);

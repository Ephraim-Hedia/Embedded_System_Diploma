...
typedef struct
{
	...
	uint32_t	BaudRate ;			//This number configures the UART communication baud rate
									//This parameter must be based on @ref UART_BaudRate_define
	...
	uint8_t		HWFlowCtl;			//Specifies whether the hardware flow control mode is enable or disabled
									//@ref UART_HwFlowCtl_define

	uint8_t		IRQ_Enable; 		//enable or disable UART IRQ Tx/Rx
									//@ref UART_IRQ_Enable_define ,you can select two or three parameters Ex.UART_IRQ_Enable
...
//to do MCAL_USART_DMA_Init()		//Multi buffer Communication
